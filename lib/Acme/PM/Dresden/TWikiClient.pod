...
This class is derived from WWW::Mechanize and extended for easy remote
controlling a TWiki based webseite.
...
Auxiliary function. Constructs URL from C<bin_url>, $cmd (view, edit,
...) and topic name. If topic name doesn't contain a web prefix (eg.,
the "Main." in "Main.WebHome"), then C<current_default_web> is used.
